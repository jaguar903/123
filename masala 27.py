...
phone = input("Hozirgi SMS bilan bog'langan telefon raqamini kiriting (+998...) : ").strip()
...
while True:
    ...
    if xizmatlar == 1:
        ...
        tasdiq = input(f"{ayir} so'm yechilsinmi? (ha/yo'q): ").strip().lower()
        if tasdiq == "ha":
            hisob -= ayir
            print(f"Operatsiya muvaffaqiyatli. Hisobingizda {hisob} so'm qoldi.")
        else:
            print("Operatsiya bekor qilindi. Hisob o'zgarmadi.")

    elif xizmatlar == 4:
        if turi == "humo":
            print("Eslatma: HUMO kartalar uchun SMS raqamini o'zgartirish cheklangan.")
            continue

        print("Uzcard aniqlandi. Telefon raqamini o'zgartirish uchun OTP yuboramiz.")
        yangi_phone = input("Yangi telefon raqamini kiriting (+998...) : ").strip()
        ...
        print("\n--- Oxirgi kiritilgan ma'lumotlar ---")
        print(f"Karta raqami: {last_card}")
        print(f"Pin kodi: {last_pin}")
        if turi == "uzcard":
            print(f"SMS bilan bog'langan telefon (oxirgi): {last_phone}")
        else:
            print("SMS bilan bog'langan telefon: (faqat Uzcard uchun saqlanadi)")
        break
